...
<div class="wrap">
	...
	<?php //echo implode('<br>', $this::get_dir_smile_names() ); ?>
	<?php //die( print_r($this->opt) ); ?>

	<form class="kwsform" method="post" action="">
		<?php wp_nonce_field('kwps_options_up', 'kwps_nonce'); ?>

		<div class="select_sm_wrap <?php echo $this->theopt('sm_pack') ?>">
			<h3><?php _e('Selected (sort by drag and drop)','kama-wp-smile'); ?></h3>
			<div class="select_smiles"><?php $this->dir_smiles_img(); ?></div>

			<?php
			$smiles = '';
			foreach( (array) $this->theopt('used_sm') as $smile )
				$smiles .= "$smile,";
			$smiles = rtrim($smiles, ',');
			?>
			<input type="hidden" name="used_sm" class="used_sm" value="<?php echo $smiles ?>">
		</div>

		<div class="kws-wrapper kws_ex_wrap" style="">
			...
			<input style="top:1em;right:0.5em;" type="radio" name="smlist_pos" value="topright"    <?php checked('topright', $this->theopt('smlist_pos') ); ?> />
			<input style="bottom:.8em;right:0.5em;" type="radio" name="smlist_pos" value="bottomright" <?php checked('bottomright', $this->theopt('smlist_pos') ); ?> />
			<input style="bottom:.8em; left:.8em;" type="radio" name="smlist_pos" value=""            <?php checked('', $this->theopt('smlist_pos') ); ?> />

			<?php echo $this->get_all_smile_html('testtx'); ?>
		</div>
		...
		<table class="kwstable">
			...
			<tr>
				<td><input type="text" name="textarea_id" value="<?php echo $this->theopt('textarea_id')?>" /></td>
				<td>
			...
			<tr>
				<td><input type="text" name="spec_tags" value="<?php echo implode(',', (array) $this->theopt('spec_tags'))?>" /></td>
				<td>
			...
			<tr>
				<td>
					...
					foreach( (array) $this->theopt('hard_sm') as $k => $v ) echo $k .' >>> '. $v ."\n";
					echo '</textarea>';
					?>
				</td>
				<td>
			...
			<tr>
				<td>
					<input type="text" name="sm_start" placeholder="(:" value="<?php echo $this->theopt('sm_start'); ?>" style="width:60px;" /> smile
					<input type="text" name="sm_end" placeholder=":)" value="<?php echo $this->theopt('sm_end'); ?>" style="width:60px;" />
				</td>
				<td>
			...
			<tr>
				<td>
					...
					if( is_dir( $path = untrailingslashit(KWS_PLUGIN_PATH) .'-packs' ) ){
						foreach( glob("$path/*") as $dir ) if( is_dir($dir) ) $packs[sanitize_key(basename($dir))] = true;
					}
					foreach( glob(KWS_PLUGIN_PATH ."packs/*") as $dir ) if( is_dir($dir) ) $packs[sanitize_key(basename($dir))] = true;
					...
					foreach( $packs as $pack ){
						$option[] = '<option '. selected($pack, $this->theopt('sm_pack'), 0) .'>'. $pack .'</option>';
					}
			...
			<tr>
				<td colspan="2">
					...
					<textarea name="additional_css" style="width:100%; height:70px;"><?php echo esc_textarea( $this->theopt('additional_css') )?></textarea>
				</td>
			</tr>
		</table>
